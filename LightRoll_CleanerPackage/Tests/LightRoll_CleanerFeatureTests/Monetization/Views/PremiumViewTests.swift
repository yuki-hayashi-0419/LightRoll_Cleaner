...
final class MockPurchaseRepository: PurchaseRepositoryProtocol {
    ...
    var shouldThrowCancelledError: Bool = false
    var shouldThrowNoSubscriptionError: Bool = false
    var fetchProductsCalled: Bool = false
    var purchaseCalled: Bool = false
    var restoreCalled: Bool = false
    ...
    func purchase(_ productId: String) async throws -> PurchaseResult {
        ...
        if shouldThrowCancelledError {
            throw PurchaseError.purchaseCancelled
        }

        if shouldThrowError {
            throw PurchaseError.purchaseFailed("Mock error")
        }

        return .pending  // Success扱い
    }

    func restorePurchases() async throws -> RestoreResult {
        restoreCalled = true

        if shouldThrowNoSubscriptionError {
            throw PurchaseError.noActiveSubscription
        }

        if shouldThrowError {
            throw PurchaseError.restorationFailed("Mock error")
        }
