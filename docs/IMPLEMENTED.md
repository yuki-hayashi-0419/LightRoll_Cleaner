...
### バージョン: 0.10.0（Phase 2 画像分析 進行中）

**アプリ機能**: 類似写真のグループ検出、顔検出（セルフィー識別）に加え、ブレ写真の自動検出機能が完成。Laplacian分散アルゴリズムにより高精度なブレ判定を実現。3つの検出感度プリセット（標準/厳格/緩和）により、ユーザーのニーズに合わせた写真整理が可能。低品質写真の整理をより効率的に実施できる。

---

## Version 0.10.0 (2025-11-28) - ブレ検出機能

### ブレ写真の自動検出
- **高精度なブレ判定**: Laplacian分散アルゴリズムによるブレスコア計算
- **画像解析プロセス**: グレースケール変換 → Laplacian畳み込み → 分散計算
- **3つの検出感度プリセット**: default（閾値100）、strict（150）、relaxed（50）
- **最適化処理**: 画像リサイズ（長辺800px）によるメモリ効率向上とパフォーマンス改善
- **詳細な分析結果**: ブレスコア、判定結果（blurry/sharp）、信頼度を取得

**技術的成果**:
- BlurDetector: ブレ検出サービス（actor実装）
- BlurAnalysisResult: ブレ分析結果モデル（スコア、判定、信頼度）
- BlurDetectionOptions: 検出感度プリセット（3種類）
- Laplacian分散アルゴリズム統合（Vision Framework + Core Graphics）
- バッチ処理、進捗通知、キャンセル対応
- 23テストケース全パス（0.043秒）、品質スコア: 107/120点
...
### M3: Image Analysis & Grouping - 進行中（8/13タスク, 61.5%）

**ユーザー視点でできるようになること**:
- **実装済み**: 類似写真を自動でグループ化（連写した写真をまとめて整理）
- **実装済み**: セルフィー（自撮り写真）を自動識別（顔検出による判定）
- **実装済み**: ブレ写真を自動検出（Laplacian分散アルゴリズムによる高精度判定）
- 準備中: スクリーンショットの自動検出（UIスクリーンショット判定）
...
- **BlurDetector**: ブレ検出サービス（Laplacian分散アルゴリズム、3プリセット）
...
| ブレ検出 | **実装完了** | BlurDetectorによるLaplacian分散ベースの高精度ブレ判定完成 |
...
| 0.10.0 | 2025-11-28 | M3: ブレ検出機能（BlurDetector + Laplacian分散アルゴリズム） |
| 1.0.0 | 予定 | 初回リリース（MVP） |

---

*最終更新: 2025-11-28 (impl-011)*
